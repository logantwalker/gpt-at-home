...
# encoding hugging face dataset
encoded_dataset = dataset.map(lambda examples: {'input_ids': encode(examples['text'])}, batched=True, remove_columns=['text'])
...
def get_batch(dataset, block_size, batch_size, device):
    # generate a small batch of data of inputs x and targets y
    ...
    for i in range(batch_size):
        try:
            ...
            # if we've reached the end of the dataset, reshuffle and restart the iterator
            dataset = iter(dataset.shuffle(seed=42))
            example = next(dataset)
        x[i, :] = example["input_ids"][:-1]
        y[i, :] = example["input_ids"][1:]
    return x, y
